...
class Clock {
  constructor(canvas = null, time = 0) {
    if (canvas !== null) {
      ...
      this.draw_clock = () => {
        throw new Error("Initialized without canvas");
      };
    }
  ...
  parseTime(str_time) {
    let validator = /(\d{1,3}:)?(\d{2}:)?(\d{2})(\.\d)?/;

    let match = validator.exec(str_time);

    if (match === null) {
      $("#error").text(
        "Enter values in the format H:MM:SS.S . You may omit hours or deciseconds."
      );
      return;
    }
  ...
  draw_clock() {
    ...
    this.ctx.fillText(
      this.timeToStr(this.time),
      Math.floor(this.pwidth * 1.25 + this.buff * 4),
      (this.pheight + this.buff) * (this.height - 1)
    );
